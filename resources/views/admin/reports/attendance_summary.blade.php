    ...
    <div class="row justify-content-center">
        <div class="col-md-10">
            <div class="card shadow">
                ...
                <div class="card-body">
                    <p class="text-muted mb-4">Generate a comprehensive attendance summary report for {{ $school->name ?? 'your school' }}. Select the parameters below to customize your report.</p>

                    <form action="{{ route('admin.reports.generate-attendance-summary') }}" method="POST" id="attendanceReportForm">
                        @csrf
                        @if($school)
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Store original sections data
        const originalSections = {};
        @foreach($sectionsByGradeLevel as $gradeLevel => $gradeSections)
            originalSections['{{ $gradeLevel }}'] = [
                @foreach($gradeSections as $section)
                {
                    id: '{{ $section->id }}',
                    name: '{{ $section->name }}'
        ...
        function updateSectionOptions() {
            ...
            const gradesToShow = selectedGrade ? [selectedGrade] : Object.keys(originalSections);
            
            gradesToShow.forEach(gradeLevel => {
                if (originalSections[gradeLevel]) {
                    const sectionsToShow = originalSections[gradeLevel];
