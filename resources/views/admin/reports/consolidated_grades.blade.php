            ...
            <div class="card shadow border-0">
                ...
                <div class="card-body pt-0">
                        ...
                        <div class="d-flex">
                            ...
                            <div>
                                ...
                                <p class="mb-0">
                                    ...
                                    Subject teachers need to approve their grades in the Grade Approvals section.
                                </p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="row mb-4">
                                ...
                                <div class="card-body py-3">
                                    ...
                                    <p class="card-text mb-0">
                                        <small class="text-muted">
                                            <i class="fas fa-map-marker-alt me-1"></i> {{ $school->address }}<br>
                                            <i class="fas fa-building me-1"></i> {{ $school->division_name }} - {{ $school->region }}
                                        </small>
                                    </p>
                                </div>
                            </div>
                        ...
                        <div class="col-md-4">
                            <label for="grade_level_filter" class="form-label">Filter by Grade Level:</label>
                    ...
                    <div class="alert alert-warning mb-4">
                        ...
                        No school has been configured in the system. Please set up a school first.
                    </div>
                    @endif

                    <!-- Generate Report Form - Opens in new tab -->
                    <form action="{{ route('admin.reports.generate-consolidated-grades') }}" method="POST" id="reportForm" target="_blank">
                        @csrf
                            ...
                            <div class="tab-pane fade {{ $loop->first ? 'show active' : '' }}" id="{{ $quarterKey }}-content" role="tabpanel" aria-labelledby="{{ $quarterKey }}-tab">
                                @if($sectionsByGradeLevel->isEmpty())
                                    <div class="alert alert-warning">
                                        <i class="fas fa-exclamation-triangle me-2"></i>
                                        No sections found. Please add sections to the schools.
                                    </div>
                                @else
                                            ...
                                            <div class="card-body p-0 section-list" id="sectionList{{ $gradeLevel }}">
                                                <div class="list-group list-group-flush">
                                                    @foreach($gradeSections as $section)
                                                        <div class="list-group-item section-item" data-section-name="{{ strtolower($section->name) }}" data-school-id="{{ $section->school_id }}">
                                                            <div class="d-flex justify-content-between align-items-center py-2">
                                                                <div>
                                                                    <h6 class="mb-0">{{ $section->name }}</h6>
                                                                    <small class="text-muted">
                                                                        <i class="fas fa-school me-1"></i> {{ $section->school->name ?? 'Unknown School' }}
                                                                        <br>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function filterSections() {
            ...
            // Show/hide grade level cards
            const gradeCards = document.querySelectorAll('.section-grade-card');
            gradeCards.forEach(card => {
                const gradeLevel = card.getAttribute('data-grade-level');
                const shouldShowGrade = !selectedGradeLevel || gradeLevel === selectedGradeLevel;
                
                if (shouldShowGrade) {
                    card.style.display = '';
                    
                    // All sections in this grade level are visible since we're using single school
