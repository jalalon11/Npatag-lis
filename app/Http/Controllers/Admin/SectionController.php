...
class SectionController extends Controller
{
    ...
    {
        try {
            // Admin can see all sections or filter by school
            $query = Section::with(['adviser', 'subjects', 'school'])
                ->withCount('students')
                ->orderByRaw('CAST(REPLACE(REPLACE(REPLACE(grade_level, "Grade ", ""), "grade ", ""), " ", "") AS UNSIGNED)');
            
            // If admin wants to filter by school
            ...
            return view('admin.sections.index', ['sections' => collect()])
    ...
    public function store(Request $request)
    {
        try {
            ...
            if ($isBatch) {
                ...
                // Validate school_id for batch operations
                $request->validate([
                ...
                foreach ($lines as $index => $line) {
                    ...
                    // Validate adviser exists and belongs to the selected school
                    ...
                    if (!$teacher) {
                        $errors[] = "Line " . ($index + 1) . ": Invalid adviser ID {$adviserId}. No teacher with this ID found in the selected school.";
                        $allValid = false;
                ...
                if (!$allValid) {
                    ...
                    if (!$allTeachersValid) {
                        ...
                        $availableTeachers = User::where('school_id', $request->school_id)
                            ->where('role', 'teacher')
                            ->get(['id', 'name']);

                        $errors[] = "Available teacher IDs in the selected school: " . $availableTeachers->map(function($teacher) {
    ...
    public function show(Section $section)
    {
        try {
            ...
            $subjects = Subject::where('school_id', $section->school_id)
            ...
            $mapehSubjects = Subject::where('school_id', $section->school_id)
                ->where('is_active', true)
                ->whereHas('components')
                ->with('components')
                ->get();
            ...
            return view('admin.sections.show', compact('section', 'teachers', 'subjects'));
        } catch (\Exception $e) {
