...
class ReportController extends Controller
{
    ...
    public function generateConsolidatedGrades(Request $request)
    {
        ...
        // Get the transmutation table from request or use default
        if ($request->has('transmutation_table')) {
            // Use the table from the request
            $preferredTableId = $request->transmutation_table;
        } else {
            // Default to DepEd Transmutation Table (1)
            $preferredTableId = 1;
        }

        return view('admin.reports.consolidated_grades_report', [
            'section' => $section,
            'quarter' => $validated['quarter'],
    ...
    public function generateAttendanceSummary(Request $request)
    {
        ...
        if (!empty($validated['school_id'])) {
            $sectionsQuery->where('school_id', $validated['school_id']);
        }
        ...
        return view('admin.reports.attendance_summary_report', [
            'summary' => $summary,
            'sections' => $sections,
            'startDate' => $startDate,
            'endDate' => $endDate,
